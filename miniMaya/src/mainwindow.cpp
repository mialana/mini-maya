...
MainWindow::MainWindow(QWidget* parent)
    : QMainWindow(parent)
    , filename(QDir().cleanPath(QString(PROJECT_SOURCE_DIR) + "/../obj_files/cube.obj"))
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->mygl,
            ...
            this,
            SLOT(slot_setSelectedVert(QListWidgetItem*)));
    ...
    connect(ui->jointsTreeWidget,
            ...
            this,
            SLOT(slot_setSelectedJoint(QTreeWidgetItem*)));
    ...
    connect(ui->triangulateFace, SIGNAL(clicked()), ui->mygl, SLOT(slot_triangulateFace()));

    connect(ui->subdivideMesh, SIGNAL(clicked()), ui->mygl, SLOT(slot_subdivideMesh()));

    connect(ui->bindSkeleton, SIGNAL(clicked()), ui->mygl, SLOT(slot_bindSkeleton()));

    connect(ui->pushButton_2, SIGNAL(clicked()), ui->mygl, SLOT(slot_exportToUSD()));

    connect(ui->vertPosXSpinBox,
            SIGNAL(valueChanged(double)),
            ui->mygl,
            SLOT(slot_translateX(double)));
    ...
    connect(ui->faceBlueSpinBox,
            SIGNAL(valueChanged(double)),
            ui->mygl,
            SLOT(slot_changeBlue(double)));

    connect(ui->jointTransXSpinBox,
            SIGNAL(valueChanged(double)),
            ui->mygl,
            SLOT(slot_jointTranslateX(double)));

    connect(ui->jointTransYSpinBox,
            SIGNAL(valueChanged(double)),
            ui->mygl,
            SLOT(slot_jointTranslateY(double)));

    connect(ui->jointTransZSpinBox,
            SIGNAL(valueChanged(double)),
            ui->mygl,
            SLOT(slot_jointTranslateZ(double)));

    connect(ui->jointRotatePlusXPushButton,
            SIGNAL(clicked()),
            ui->mygl,
            SLOT(slot_jointRotatePlusX()));

    connect(ui->jointRotatePlusYPushButton,
            SIGNAL(clicked()),
            ui->mygl,
            SLOT(slot_jointRotatePlusY()));

    connect(ui->jointRotatePlusZPushButton,
            SIGNAL(clicked()),
            ui->mygl,
            SLOT(slot_jointRotatePlusZ()));

    connect(ui->jointRotateMinusXPushButton,
            SIGNAL(clicked()),
            ui->mygl,
            SLOT(slot_jointRotateMinusX()));

    connect(ui->jointRotateMinusYPushButton,
            SIGNAL(clicked()),
            ui->mygl,
            SLOT(slot_jointRotateMinusY()));

    connect(ui->jointRotateMinusZPushButton,
            SIGNAL(clicked()),
            ui->mygl,
            SLOT(slot_jointRotateMinusZ()));
}
...
void MainWindow::on_actionImport_Obj_triggered()
{
    QString validate_filename = QFileDialog::getOpenFileName(
        ...
        QDir().cleanPath(QFileInfo(PROJECT_SOURCE_DIR).absoluteFilePath() + "/../obj_files"),
...
{
    QString validate_file = QFileDialog::getOpenFileName(0,
                                                         QString("Select JSON File"),
                                                         QDir().cleanPath(QString(PROJECT_SOURCE_DIR)
                                                                          + "/../jsons"),
                                                         QString("*.json"));
