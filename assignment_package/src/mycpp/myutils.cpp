...
void startup::setOpenGLFormat() {
  ...
  format.setOption(QSurfaceFormat::DeprecatedFunctions,
                   false); // deprecated gl not supported
  ...
  format.setSamples(
      samples); // for 4-sample antialiasing. not always supported.
...
void startup::_customizeQDebugHandler(QtMsgType type,
  ...
  QTextStream locStream(
      &location); // streams are easier to append different types
  ...
  QString outFilePath =
      QString("/Users/liu.amy05/Documents/Neural-for-USD") + "/log.txt";
...
void startup::setUpCustomLogging() {
  ...
  qWarning()
      << "Use the `qWarning()` macro to print a warning message to console.";
  qCritical()
      << "Use the `qCritical()` macro to print a critical message to console.";
  qInfo() << "Use the `qFatal()` macro to instantly abort your program and "
             "print a fatal message "
             "to console.";
  qInfo() << "Append the above macros with `.Nq()` to remove extra quotes in a "
             "console message.";
  qInfo() << "Append the above macros with `.Ns()` to remove extra spaces in a "
             "console message.";
}

void startup::doSimpleSetup() {
  console::createFunctionSection<void, void (*)()>(
      "INSTRUCTIONS FOR CUSTOM LOGGING:", startup::setUpCustomLogging);
...
QRgb linAlg::mapDirectionToRgb(const Vector3f &d) {
  ...
  myQRgb =
      qRgb(col255.r, col255.g, col255.b); // `qRgb()` is QT convenience function
...
Point3f linAlg::doPoint3fXMat4(const glm::mat4 &mat, const glm::vec3 &pt) {
  Point4f homogenized =
      glm::vec4(pt, 1); // points must apply translation column
...
Vector3f linAlg::doVec3fXMat4(const glm::mat4 &mat, const glm::vec3 &vec) {
  Vector4f homogenized =
      glm::vec4(vec, 0); // vectors disregard translation column
  Vector4f transformed = mat * homogenized;

  Vector3f result =
      transformed.xyz(); // exp(w) = 0. Do not normalize to cover all use cases.
  return result;
}
